    ...
    for c in cams:
        ...
        result.append({
            ...
            **push_target,
            ...
            "today_raw_event_count": 0,
            "today_unknown_event_count": 0,
        })
    db.commit()
    return result
...
def get_camera(request: Request, cam_id: int, db: Session = Depends(get_db)):
    ...
    isup_available = source != "unavailable"
    isup_online = None
    info = _find_live_device_for_camera(cam, isup_map) if isup_available else None
    if info is not None:
        isup_online = _is_live_device_online(info)
    elif isup_available and cam.isup_device_id:
        isup_online = False
    dynamic_online = (
        isup_online if isup_online is not None
        else bool(cam.last_seen_at and (now - cam.last_seen_at) <= online_threshold)
    )

    # Bitta kamera uchun bugungi so'rov
