...
import { ArrowClockwiseRegular, PeopleRegular } from '@fluentui/react-icons'
...
export default function Tracking() {
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter(emp => {
      ...
      const queryMatch = name.includes(searchQuery.toLowerCase()) || 
                         (emp.personal_id || '').toLowerCase().includes(searchQuery.toLowerCase())
      if (showOnlyOnDuty) {
        return queryMatch && emp.in_working_hours
      }
      return queryMatch
    })
  }, [employees, searchQuery, showOnlyOnDuty])
      ...
      <div style={{
        ...
        <div style={{ padding: 16, borderBottom: '1px solid var(--border-2)' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 12, color: 'var(--text-2)' }}>
            <input
              type="checkbox"
              checked={showOnlyOnDuty}
              onChange={(e) => setShowOnlyOnDuty(e.target.checked)}
            />
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span style={{
                      ...
                      height: 8,
                      borderRadius: '50%',
                      background: emp.in_working_hours ? '#10b981' : '#9ca3af',
                      boxShadow: emp.in_working_hours ? '0 0 6px #10b981' : 'none'
                    }} />
                  </div>
      ...
      <div style={{ flex: 1, position: 'relative', zIndex: 1 }}>
        ...
        >
          ...
          {employees.map(emp => {
            ...
            const pulseIcon = createPulseIcon(emp.in_working_hours ? '#10b981' : '#fbbf24')
            const name = `${emp.first_name || ''} ${emp.last_name || ''}`
            return (
              <Marker
                key={`emp-${emp.id}`}
              ...
              >
                <Popup>
                  <div style={{ fontFamily: 'system-ui', fontSize: 12, minWidth: 160 }}>
                    <strong style={{ display: 'block', marginBottom: 3, fontSize: 13 }}>{name}</strong>
                    <span style={{ color: 'var(--text-3)' }}>{emp.department} • {emp.position}</span><br />
                    <span style={{ color: 'var(--text-3)' }}>{isRu ? 'Рабочее время:' : 'Ish vaqti:'} {emp.work_time}</span><br />
                    {emp.last_location_time && (
                      <span style={{ color: 'var(--text-4)', fontSize: 10 }}>
                        {isRu ? 'Активен:' : 'Oxirgi faollik:'} {new Date(emp.last_location_time).toLocaleString()}
                      </span>
                    )}
                  </div>
                </Popup>
